...
The nature of the spikes are quite interesting.  The same pattern appears on both the Samsung supply and the laptop supply (but to a far lesser extent on the laptop).  We can therefore conclude that it is caused by the nodeMCU board.  The deep spikes occurs every 20 seconds or so. Perhaps there is some process taking place at this interval that sinks a significant current in the ESP8266?

At this point I changed over to a small 5V, 700 mA [AC/DC switched mode supply](http://www.ebay.com/itm/AC-DC-5V-700mA-3-5W-Power-Supply-Buck-Converter-Step-Down-Module-for-Arduino-/281869225417).  This shows the breadboard setup. The three PIR sensors are underneath the mugs to prevent legitimate triggering.  
The no-load supply voltage was within spec and the ripple was reasonably small (less than 20 mV), but with occasional very short spikes.

![5V700mA-supply.jpg](images/5V700mA-supply.jpg)


![PIR3-alarm02.jpg](images/PIR3-alarm02.jpg)

With the new power supply connected to the nodeMCU, the performance was quite poor, even after decoupling with 150 nF and 100 uF capacitors. I measured the current consumption of the nodeMCU board by placing a 1.5 Ohm series resistor in the supply line. The following graph shows a time recording of the current: average value around 271mV/1.5 Ohm=180 mA, which is in agreement with what is published elsewhere. However, there were relatively long spikes of 100mW/1.5Ohm=65mA peak-to-peak. The maximum current was 215 mA, nowhere near the 500 to 700 mA of some of the horror stories told on the Internet.   Clearly the PIR might be triggered by these large peaks! It later turned out that it was not these large obvious spikes that caused the trigger, but other smaller spikes that were not visible on these low frequency time recordings.

![charger-supply-current-1_5ohm.JPG](images/charger-supply-current-1_5ohm.JPG)

Side note:  these measurement were taken on a cluttered work area with leads all over the place. Also ground wires were small diameter and long - hardly good ground implementations.  Some of the interference should disappear when the circuit is properly assembled with short leads and good low-impedance grounding.  I suspect that the spectra shown may also be distorted by erratic triggering on the scope.

To verify that it was the power supply that caused the false alarms, I again powered the nodeMCU with PIR sensors from my notebook USB port. There where no false alarms.  To verify that the PIR sensors are not overly sensitive to RF interference, I placed the AC/DC switch mode power supply right next to the PIR sensors. Still no false alarms (at least not during my short experiment). The following graph shows the supply voltage after 5 V to 3.3 V down regulation on the nodeMCU board, when powered from my notebook. Note the low frequency content in the signal spectrum, also there were no spikes on the supply voltage.

![charger-supply-voltage07-laptop-supply.JPG](images/charger-supply-voltage07-laptop-supply.JPG)

When powered from the switch mode supply with no decoupling capacitors, there were many false alarms.  Note the severe spikes on the power supply, occurring every second or so. Note also the large low frequency noise spectrum.  It is clear that the combined forces of the switch mode power supplies and the ESP8266 erratic supply needs were too much of the sensitive electronics of the PIR detector.

![charger-supply-voltage07-SMPSno-decoupling.JPG](images/charger-supply-voltage07-SMPSno-decoupling.JPG)

Adding decoupling capacitors to the output of the 5V power supply and the nodeMCU down-regulated 3.3 V helped somewhat, but there were still far too many false alarms. All/most PIR sensors triggered at the same time, rendering my false alarm rejection logic useless.  The spikes still remained. Even though the  capacitors suppressed some of the high frequency noise, the low frequency noise persisted.

![charger-supply-voltage07-SMPS-with-decoupling.JPG](images/charger-supply-voltage07-SMPS-with-decoupling.JPG)

No measure of decoupling could reject the noise and protect my PIR sensors, so I decided to use a separate 3.3 V regulator, but still supplied from the same 5V supply that powers the nodeMCU (in other words not to use the on-board nodeMCU regulator).  The input and output of the LM1117AL-3.3V linear regulator were decoupled by large electrolytic and small decoupling capacitors  (100 uF and 150 nF). The PIR alarms did not trigger falsely on the new regulated power supply (actually down to once per six hours, which is fine for my false alarm rejection logic).  The final graphs shows the power supply noise after linear regulation and decoupling.  Note that the spikes are much reduced (effectively gone, except for the occasional weak spike) and that the noise spectrum is much smaller (both low and high frequencies) than before.
...
My conclusion is now that the sensitive PIR detector picks up sufficient noise to trigger from the power supply (created by the ESP) and not from general RF interference. With a clean power supply the alarm false triggers all but disappeared. Once deployed the PIRs will be about four to five metres from the power supply, and it is expected that the long leads will further suppress the high frequency spikes.  It would also make sense to put an inductive filter (ferrite bead) in the positive power supply lead only (not the ground, it will shift the ground when high frequency signals are present). Place a capacitor on the load side of the inductor to provide storage for transient load current needs (the inductor will suppress the transient flow in the positive supply).

It was also clear that the DuPont pin and wiring and non-solder breadboard mechanism is not very stable.  At times the false alarm rate was high and after some fiddling with the wires, it disappeared.  A proper stable and low-impedance soldered joint is essential when it comes to interference control.  You may want to look at [this excellent paper](http://www.murata.com/~/media/webrenewal/support/library/catalog/products/emc/emifil/c39e.ashx).
