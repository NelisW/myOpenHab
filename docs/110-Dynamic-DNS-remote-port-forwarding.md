...
1.  The RPi running on a local wifi network only has visibility within the network itself, it is not visible on the internet.  The wifi router connects the local servers to the internet so the router must be set up properly to connect the local server on the local network to the internet.  
...
5. For the purpose of this document we assume you want to make an HTTP server on your Raspberry Pi visible on the internet.  This server could serve blog pages or provide control of some service (via HTTP) on your internal network.

Note that in all of the work below remember that the changes are not instantaneous, you may have to wait a short time for all the setting values to propagate to the appropriate servers.
...
1. Once logged into the interface, navigate to `DynDNS Pro`. The page should show your account details including currently registered DYNDNS host names and the IP address currently linked to the hostname.  You can add a new hostname by clicking on `+ Add New Hostname`. A page will appear that allows you to enter your preferred hostname, current IP address and the type of service.

	1. Type in your preferred name. For example `savvyPetesBlog` and select the rest of the domain name from the dropout box, e.g., `is-lost.org`.  This means that your internet-visible address will be `savvyPetesBlog.is-lost.org`
	...
	5. Click on `Add to Cart`. This should add your new hostname against the currently indicated IP address (which may change in future).
...
Edit the file to contain at least the following (if you are using a DynDNS service):
	...
	ssl=yes
	# your www.dyndns.com login details
	...
	# the URL(s) provided by dyndns
	<your-dynDNS-hostname>.dyndns.org
...
https://help.dyn.com/remote-access/getting-started-with-remote-access/  
...
On my Netgear N300/DGN2200Mv2 router there is no explicit port forwarding functionality.  There are services and and firewall rules working together as set out here below.  In the example below we set up the Raspberry HTTP server to run on port 888 (replace with your own port number). This is not traditional port forwarding.
